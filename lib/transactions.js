...
// Meter: reference node validation.cpp 
// check for scriptSig must be adjusted for meter project
// scriptSig contains 4 segments: 
// encoded Height (vaired length) + Seqence (4 bytes) + TimeStamp (8 bytes) + Beneficiary str(40 bytes)  
exports.CreateGeneration = function (rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, beneficiary) {
    ...
    console.log('**************' + JSON.stringify(rpcData))

    var flags = rpcData.coinbaseaux.flags !== undefined ? rpcData.coinbaseaux.flags : []

    var scriptSigPart1 = Buffer.concat([
        util.serializeNumber(rpcData.height),
        new Buffer.from(flags, 'hex'),
        util.serializeNumber(Date.now() / 1000 | 0),
        new Buffer.from([extraNoncePlaceholder.length])
    ]);

    var scriptSigPart2 = util.serializeString(beneficiary);
